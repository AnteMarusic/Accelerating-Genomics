...
int main() {
    ...
    strncpy(sx, "ATGGGCTACTTACGCGGTACTA", sizeof(sx) - 1);  // Safe copy with null terminator
    sx[sizeof(sx) - 1] = '\0';           // Ensure null termination
    strncpy(sy, "ATCGGATTGACTCGTGCGGTACGCAGG", sizeof(sy) - 1);  // Safe copy with null terminator
    sy[sizeof(sy) - 1] = '\0';           // Ensure null termination
    // Remove trailing newline characters (if present)
    sx[strcspn(sx, "\n")] = '\0';
    sy[strcspn(sy, "\n")] = '\0';
    int sx_len =  strlen(sx) >= strlen(sy) ? strlen(sy) : strlen(sx);
    int sy_len = strlen(sx) <= strlen(sy) ? strlen(sy) : strlen(sx);

    if (sx_len > sy_len) {
        strncpy(stmp, sx, sx_len - 1);
        strncpy(sx, sy, sy_len -1);
        strncpy(sy, stmp, sx_len -1);
    }
