...
int m_get(int *val, int iy, int ix, int mat, int *antidiags, int cur_antid_num, int nx, int ny) {
    ...
    *val = antidiags[index];
    PRINT("[m_get] getting val: %d, at iy:%d, ix:%d, on matrix: %d, at index: %d\n", *val, iy, ix, mat, index);
    return 1;
}
...
int m_set(int *val, int iy, int ix, int mat, int *antidiags, int cur_antid_num, int nx, int ny) {
    ...
    PRINT("[m_set] setting val: %d, iy:%d, ix:%d, at index: %d on matrix %d\n", *val, iy, ix, index, mat);
    antidiags[index] = *val;
    return 1;
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < line_num; i+=2) {
        ...
        if(strlen(line1) > strlen(line2)){
            sx_len = strlen(line2);
            sy_len = strlen(line1);
            sx = (char *)malloc(sx_len+1*sizeof(int));
            sy = (char *)malloc(sy_len+1*sizeof(int));
            strncpy(sx, line2, sx_len);
            strncpy(sy, line1, sy_len);
        }
        else{
            sx_len = strlen(line1);
            sy_len = strlen(line2);
            sx = (char *)malloc(sx_len+1*sizeof(int));
            sy = (char *)malloc(sy_len+1*sizeof(int));
            strncpy(sx, line1, sx_len);
            strncpy(sy, line2, sy_len);
        }
